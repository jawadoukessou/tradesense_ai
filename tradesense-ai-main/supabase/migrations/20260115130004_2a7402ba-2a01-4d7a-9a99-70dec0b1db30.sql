-- Drop and recreate the view with security_invoker to fix security issue
...
-- Grant select access for public leaderboard
GRANT SELECT ON public.monthly_leaderboard TO authenticated;
GRANT SELECT ON public.monthly_leaderboard TO anon;

-- Add RLS policy to profiles to allow reading full_name for leaderboard (public data)
CREATE POLICY "Anyone can view profile names for leaderboard"
ON public.profiles
FOR SELECT
USING (true);